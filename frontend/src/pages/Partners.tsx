      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {partners.map((partner, index) => (
            <div
              key={partner.id}
              className="glass-card p-6 animate-fade-in hover:border-primary-500/30 transition-colors"
            ...
            >
              ...
              <div className="mt-4 pt-4 border-t border-dark-700/50 flex items-center justify-between">
                {partner.website ? (
                  ...
                  <span className="text-sm text-dark-500">Нет сайта</span>
                )}
                <span className="text-xs text-dark-500">
                  {format(new Date(partner.created_at), 'd MMM yyyy', { locale: ru })}
                </span>
              </div>
            </div>
          ))}
